...
public class UnethicalMotherlodePlugin extends LoopedPlugin
{
    ...
    private static final Set<Integer> VALID_DEPOSIT_IDS = ImmutableSet.of(436, 438, 440, 442, 444, 446, 447, 449, 451, 453);
    ...
    private boolean handleCollectOre()
    {
        ...
        TileObject emptySack = TileObjects.getNearest(x -> x.getId() == FILLED_SACK_ID);
        if (emptySack != null)
        {
            log.info("empty sack exists so not collecting");
            return false;
        }
        TileObject filledSack = TileObjects.getNearest(x -> x.getId() == FILLED_SACK_ID);
        if (filledSack == null)
        {
    ...
    private boolean handleFixWheels()
    {
        ...
        TileObject brokenStrut = TileObjects.getNearest(x -> x.getId() == BROKEN_STRUT_ID);
        if (brokenStrut == null)
        {
    ...
    private boolean handleDumpPayDirt()
    {
        ...
        TileObject hopper = TileObjects.getNearest(x -> x.getId() == HOPPER_ID);
        if (hopper == null)
        {
    ...
    {
        Player local = client.getLocalPlayer();
        if (local.isAnimating() || local.isMoving() || Inventory.isFull())
            return false;

        TileObject nearestMinable = TileObjects.getNearest(x -> x.hasAction("Mine")
                && VALID_MINABLE_IDS.contains(x.getId())
                && Reachable.isInteractable(x));
